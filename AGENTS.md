...
refresh, name/ID-to-catalog resolution that auto-substitutes a fuzzy match or
trusts a stale ID over the requested name, model-tier routing changes that gut a
whole classifier or ignore the fallback tier/per-provider model policy, input
validators that reject the malformed value a downstream clamp is meant to
repair). Skim the matching section before touching Tonal fetch helpers,
...
steps, retry/fallback and normalization paths, movement/catalog resolvers,
model-tier routing, or input validators upstream of an external write -- and
