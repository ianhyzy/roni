...
without their tool schema that skip validating their own input, and Node-only
globals (`Buffer`, `process`, `fs`) used in Convex default-runtime files --
masked by widening `convex/tsconfig.json` -- which throw at runtime and silently
disable the caching path that swallows them). Skim the matching section before
...
paths, AI tool-call name/ID resolution, tool-input (Zod) schemas, internal
actions callable without the tool schema, model-tier routing, or Convex runtime
boundaries (Node globals in default-runtime files, tsconfig type widening) --
