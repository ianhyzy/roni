...
`tsconfig` changes that add ambient Node types project-wide and hide Node-only
API misuse in default-runtime files). Skim the
...
internal actions callable without the tool schema, model-tier routing, or the
Convex tsconfig/ambient type environment -- and
