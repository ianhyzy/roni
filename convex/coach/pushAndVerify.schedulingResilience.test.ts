...
function makeHarness(options: HarnessOptions = {}) {
  ...
  const runQuery = vi.fn(async (ref: TestFunctionReference) => {
    ...
    if (name === "weekPlans:getWeekPlanById") {
      ...
      return {
        _id: WEEK_PLAN_ID,
        weekStartDate: FUTURE_WEEK_START,
    ...
    if (name === "workoutPlans:getById") {
      ...
      return {
        ...
        status: lookupStatus,
        ...(lookupStatus === "pushed" ? { tonalWorkoutId: "tonal-workout-existing" } : {}),
      };
    }
    throw new Error(`Unexpected query ${name}`);
  });
  const runAction = vi.fn(async (ref: TestFunctionReference) => {
    const name = getFunctionName(ref);
...
describe("pushWeekPlanToTonal scheduling resilience", () => {
