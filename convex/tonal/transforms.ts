...
/** Compares the compact intended payload with the sets Tonal actually stored. */
export function computePushDivergence(
  ...
  for (const item of intended) {
    intendedCounts.set(item.movementId, (intendedCounts.get(item.movementId) ?? 0) + item.sets);
  }
  const storedCounts = new Map<string, number>();
  for (const set of storedSets) {
    storedCounts.set(set.movementId, (storedCounts.get(set.movementId) ?? 0) + 1);
  }
...
export function correctDurationRepsMismatch(
  ...
  const catalogMap = new Map(catalog.map((movement) => [movement.id, movement]));
  let corrections = 0;
