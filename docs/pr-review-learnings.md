...
## 12. Keep the input schema permissive and repair invalid values before the external write, so the normalization covers every push path — not just some

**Seen in:** #460 (1 P2 review thread)

**Problem.** The fix for non-positive reps/duration reaching Tonal (#447) added a
`positiveOr` clamp inside `buildTonalWorkoutSets`, but the `create_workout` tool's
Zod input schema still validated reps/duration with `.positive()`. Tool input is
validated **before** `execute` runs, so on the standalone one-off push path the
coach's malformed `reps: 0` / negative `duration` was rejected at the schema
boundary and never reached the new clamp. The retry and week-plan paths were
repaired; the direct `create_workout` path still failed on the exact input the PR
existed to fix.

**Why it matters.** When a normalization/repair step is meant to absorb fallible
LLM output before an external API, an upstream `.positive()`/`.min()` validator
that rejects the same malformed value defeats the repair on whichever path runs
the validator first. The feature ships "fixed," but the originally-reported path
still bounces — and the gap is invisible because the other paths pass.

**Preventive checks.**

- When you add a clamp/normalize step to repair fallible external/LLM input before
  a write, **make the input schema permissive enough to let the invalid value
  through to the repair** (`int().optional()` rather than `.positive()`), and
  normalize in one shared place all paths funnel through (`buildSet` /
  `buildTonalWorkoutSets`).
- **Enumerate every path that constructs the payload** (one-off tool call, retry,
  week-plan/batch) and confirm the repair sits downstream of all of them — a fix
  verified only on the retry/batch path can leave the direct path broken.
- Add coverage that drives the **originally-reported malformed input through the
  standalone path**, not just the batch path, asserting the clamp produced a valid
  payload.
...
- Before opening a PR that touches **Tonal fetch helpers, AI cost/budget paths,
  ...
  (primary vs. fallback tier, per-provider model policy), or input
  validators/normalization upstream of an external write**, skim the matching
  section above.
