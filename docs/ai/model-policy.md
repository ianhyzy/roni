...
The table shows base rates. For `gpt-5.4` and every GPT-5.6 tier, OpenAI charges the full request at 2x the input rates and 1.5x the output rate when input exceeds 272,000 tokens. GPT-5.6 cache writes remain 1.25x uncached input, so their write rate doubles too. OpenAI publishes no distinct cache-write rate for the GPT-5.4 family, so Roni conservatively uses each model's uncached input rate as its write proxy. The long-context uplift applies only to main GPT-5.4, making that proxy $2.50 at base context and $5.00 at long context; GPT-5.4 mini and nano retain their base rates. Roni applies the long-context rates to direct OpenAI requests and explicit OpenAI models routed through OpenRouter. At that input size, missing, automatic, or unknown OpenAI/OpenRouter model IDs use the long-context version of Roni's conservative known-rate maximum.
...
| `gpt-5.4-nano`          |                   $0.20 |                   $0.02 | Conservative proxy: $0.20 |                    $1.25 |
| `gpt-5.4-mini`          |                   $0.75 |                  $0.075 | Conservative proxy: $0.75 |                    $4.50 |
| `gpt-5.4`               |                   $2.50 |                   $0.25 | Conservative proxy: $2.50 |                   $15.00 |
| `openrouter/auto`       | Conservative cap: $5.00 | Conservative cap: $0.50 |   Conservative cap: $6.25 | Conservative cap: $30.00 |
...
| OpenAI     |                 $199.92050 | Cache write            |           $200.00 |
| OpenRouter |                 $199.92050 | Cache write            |           $200.00 |

These thresholds are runaway guards, not hard caps, full-attempt guarantees, or expected spend. The context builder initially targets at most 500,000 estimated text tokens after reserving output and tool-definition headroom. The reference scenario adds 25% input-growth allowance (625,000 billed input tokens per step), then prices all 25 allowed steps at the 4,096-token output limit using conservative known rates, including OpenAI's long-context adjustment. Behavior tests drive the real stop condition for uncached input, cache reads, and cache writes under that scenario.
...
Each retry or fallback is a fresh attempt with a fresh threshold, so one user turn can cross the threshold in more than one attempt. Users can store an independent threshold from $0.01 through $400.00 for each provider; the upper bound is twice the highest default while retaining typo protection. Settings edits the currently selected provider's threshold. **Ignore budget for all providers** is one global preference and the explicit option to remove the cost stop condition from every personal provider. Provider thresholds remain stored while the global toggle is on and resume when it is turned off. Shared hosted AI does not use this personal-key guard.
